...
def scrape(url):
    ...
    if req.status_code > 500:
        if "Contact for access to amazon data." in req.text:
            print(
                "Amazon Page: %s was blocked by Amazon. Please try using better proxies\n"
                % url
            )
        else:
            print(
                "Amazon Page: %s was blocked, status code is: %d"
                % (url, req.status_code)
            )
        return None
...
with open("search_products.txt", "r") as urllist, open(
    "product_search_output.jsonl", "w"
) as outfile:
